...
class RUNBoostedAnalysis : public EDAnalyzer {
	...
	private:
		...
		float AK4HT = 0, HT = 0, trimmedMass = -999, 
		  ...
		  subjet11SFUp = 0, subjet12SFUp = 0, subjet21SFUp = 0, subjet22SFUp = 0,
		  subjet11SFDn = 0, subjet12SFDn = 0, subjet21SFDn = 0, subjet22SFDn = 0,
		  subjet11Flavor = 0, subjet12Flavor = 0, subjet21Flavor = 0, subjet22Flavor = 0,

		  jet1CSVv2SF = 0, jet2CSVv2SF = 0, jet1Flavor = 0, jet2Flavor = 0,
		  jet1CSVv2SFUp = 0, jet2CSVv2SFUp = 0, jet1CSVv2SFDn = 0, jet2CSVv2SFDn = 0,
		  jet1CMVAv2SF = 0, jet2CMVAv2SF = 0,
		  jet1CMVAv2SFUp = 0, jet2CMVAv2SFUp = 0, jet1CMVAv2SFDn = 0, jet2CMVAv2SFDn = 0,
...
RUNBoostedAnalysis::RUNBoostedAnalysis(const ParameterSet& iConfig):
	...
	jetFlavor_(consumes<vector<float>>(iConfig.getParameter<InputTag>("jetFlavor"))),
	...
	genPart_(consumes<vector<reco::GenParticle>>(iConfig.getParameter<InputTag>("genPart")))
...
void RUNBoostedAnalysis::analyze(const Event& iEvent, const EventSetup& iSetup) {
	...
	Handle<vector<float> > jetFlavor;
	iEvent.getByToken( jetFlavor_, jetFlavor );
	...
	iEvent.getByToken( genPart_, genPart );
	...
	for (size_t i = 0; i < jetPt->size(); i++) {
		...
		if( ( corrJet.Pt() > cutAK8jetPt ) && jetId ) { 
			  ...
			  cout << 3.61 << endl;
				if( j == (*jetSubjetIndex0)[i] ) {
				  cout << 3.62 << endl;
					...
					cout << 3.65 << endl;
					if ( subjetFlavor->size() > j ) tmpSubjet0PartonFlavor = (*subjetFlavor)[j];
					else tmpSubjet0PartonFlavor = -999;
				...
				if( j == (*jetSubjetIndex1)[i] ) {
				  cout << 3.67 << endl;
					...
					if ( subjetFlavor->size() > j ) tmpSubjet1PartonFlavor = (*subjetFlavor)[j];
					else tmpSubjet1PartonFlavor = -999;
			...
			myJet tmpJET;
			tmpJET.p4 = corrJet;
			if ( jetFlavor->size() > i ) tmpJET.flavor = (*jetFlavor)[i];
			...
			tmpJET.subjet1PartonFlavor = tmpSubjet0PartonFlavor;
	...
	if ( ORTriggers ) {
		...
		if ( JETS.size() > 1 ) {
			...
			BTagCalibration calib2("cmva", "CMVAV2.csv");
			...
			BTagCalibrationReader reader2(BTagEntry::OP_LOOSE,  // operating point
						     "central",             // central sys type
						     {"up", "down"});      // other sys types
			
			reader2.load(calib2,                // calib2ration instance
				    BTagEntry::FLAV_B,    // btag flavour
			...
			if( JETS[0].flavor > 4 ) {
			  jet1CSVv2SF = reader.eval_auto_bounds( "central", BTagEntry::FLAV_B, JETS[0].p4.Eta(), JETS[0].p4.Pt() );
			  jet1CSVv2SFUp = reader.eval_auto_bounds( "up", BTagEntry::FLAV_B, JETS[0].p4.Eta(), JETS[0].p4.Pt() );
			  jet1CSVv2SFDn = reader.eval_auto_bounds( "down", BTagEntry::FLAV_B, JETS[0].p4.Eta(), JETS[0].p4.Pt() );
			  jet1CMVAv2SF = reader2.eval_auto_bounds( "central", BTagEntry::FLAV_B, JETS[0].p4.Eta(), JETS[0].p4.Pt() );
			  jet1CMVAv2SFUp = reader2.eval_auto_bounds( "up", BTagEntry::FLAV_B, JETS[0].p4.Eta(), JETS[0].p4.Pt() );
			  jet1CMVAv2SFDn = reader2.eval_auto_bounds( "down", BTagEntry::FLAV_B, JETS[0].p4.Eta(), JETS[0].p4.Pt() );
			}
			else if( 5. > JETS[0].flavor && JETS[0].flavor > 0. ) {
			  jet1CSVv2SF = reader.eval_auto_bounds( "central", BTagEntry::FLAV_C, JETS[0].p4.Eta(), JETS[0].p4.Pt() );
			  jet1CSVv2SFUp = reader.eval_auto_bounds( "up", BTagEntry::FLAV_C, JETS[0].p4.Eta(), JETS[0].p4.Pt() );
			  jet1CSVv2SFDn = reader.eval_auto_bounds( "down", BTagEntry::FLAV_C, JETS[0].p4.Eta(), JETS[0].p4.Pt() );
			  jet1CMVAv2SF = reader2.eval_auto_bounds( "central", BTagEntry::FLAV_C, JETS[0].p4.Eta(), JETS[0].p4.Pt() );
			  jet1CMVAv2SFUp = reader2.eval_auto_bounds( "up", BTagEntry::FLAV_C, JETS[0].p4.Eta(), JETS[0].p4.Pt() );
			  jet1CMVAv2SFDn = reader2.eval_auto_bounds( "down", BTagEntry::FLAV_C, JETS[0].p4.Eta(), JETS[0].p4.Pt() );

			  }
			else {
			  jet1CSVv2SF = reader.eval_auto_bounds( "central", BTagEntry::FLAV_UDSG, JETS[0].p4.Eta(), JETS[0].p4.Pt() );
			  jet1CSVv2SFUp = reader.eval_auto_bounds( "up", BTagEntry::FLAV_UDSG, JETS[0].p4.Eta(), JETS[0].p4.Pt() );
			  jet1CSVv2SFDn = reader.eval_auto_bounds( "down", BTagEntry::FLAV_UDSG, JETS[0].p4.Eta(), JETS[0].p4.Pt() );
			  jet1CMVAv2SF = reader2.eval_auto_bounds( "central", BTagEntry::FLAV_UDSG, JETS[0].p4.Eta(), JETS[0].p4.Pt() );
			  jet1CMVAv2SFUp = reader2.eval_auto_bounds( "up", BTagEntry::FLAV_UDSG, JETS[0].p4.Eta(), JETS[0].p4.Pt() );
			  jet1CMVAv2SFDn = reader2.eval_auto_bounds( "down", BTagEntry::FLAV_UDSG, JETS[0].p4.Eta(), JETS[0].p4.Pt() );

			}
			cout << 5 << endl;

			BTagCalibration calibsubjets("csvv1", "CSVV2_subjets.csv");
			BTagCalibrationReader readersubjets(BTagEntry::OP_LOOSE,  // operating point
						     "central",             // central sys type
						     {"up", "down"});      // other sys types
			
			readersubjets.load(calibsubjets,                // calibration instance
				    BTagEntry::FLAV_B,    // btag flavour
				    "lt");               // measurement type	
			...
			if( JETS[0].subjet0PartonFlavor > 4 ) {
			  subjet11SF = readersubjets.eval_auto_bounds( "central", BTagEntry::FLAV_B, JETS[0].subjet0.Eta(), JETS[0].subjet0.Pt() );
			  subjet11SF = readersubjets.eval_auto_bounds( "up", BTagEntry::FLAV_B, JETS[0].subjet0.Eta(), JETS[0].subjet0.Pt() );
			  subjet11SF = readersubjets.eval_auto_bounds( "down", BTagEntry::FLAV_B, JETS[0].subjet0.Eta(), JETS[0].subjet0.Pt() );
			}
			else if( 5. > JETS[0].subjet0PartonFlavor && JETS[0].subjet0PartonFlavor > 0. ) {
			  subjet11SF = readersubjets.eval_auto_bounds( "central", BTagEntry::FLAV_C, JETS[0].subjet0.Eta(), JETS[0].subjet0.Pt() );
			  subjet11SF = readersubjets.eval_auto_bounds( "up", BTagEntry::FLAV_C, JETS[0].subjet0.Eta(), JETS[0].subjet0.Pt() );
			  subjet11SF = readersubjets.eval_auto_bounds( "down", BTagEntry::FLAV_C, JETS[0].subjet0.Eta(), JETS[0].subjet0.Pt() );
			}
			else {
			  subjet11SF = readersubjets.eval_auto_bounds( "central", BTagEntry::FLAV_UDSG, JETS[0].subjet0.Eta(), JETS[0].subjet0.Pt() );
			  subjet11SF = readersubjets.eval_auto_bounds( "up", BTagEntry::FLAV_UDSG, JETS[0].subjet0.Eta(), JETS[0].subjet0.Pt() );
			  subjet11SF = readersubjets.eval_auto_bounds( "down", BTagEntry::FLAV_UDSG, JETS[0].subjet0.Eta(), JETS[0].subjet0.Pt() );
			}

			cout << 7 << endl;
			if( JETS[0].subjet1PartonFlavor > 4 ) {
			  subjet12SF = readersubjets.eval_auto_bounds( "central", BTagEntry::FLAV_B, JETS[0].subjet1.Eta(), JETS[0].subjet1.Pt() );
			  subjet12SF = readersubjets.eval_auto_bounds( "up", BTagEntry::FLAV_B, JETS[0].subjet1.Eta(), JETS[0].subjet1.Pt() );
			  subjet12SF = readersubjets.eval_auto_bounds( "down", BTagEntry::FLAV_B, JETS[0].subjet1.Eta(), JETS[0].subjet1.Pt() );
			}
			else if( 5. > JETS[0].subjet1PartonFlavor && JETS[0].subjet1PartonFlavor > 0. ) {
			  subjet12SF = readersubjets.eval_auto_bounds( "central", BTagEntry::FLAV_C, JETS[0].subjet1.Eta(), JETS[0].subjet1.Pt() );
			  subjet12SF = readersubjets.eval_auto_bounds( "up", BTagEntry::FLAV_C, JETS[0].subjet1.Eta(), JETS[0].subjet1.Pt() );
			  subjet12SF = readersubjets.eval_auto_bounds( "down", BTagEntry::FLAV_C, JETS[0].subjet1.Eta(), JETS[0].subjet1.Pt() );
			}
			else {
			  subjet12SF = readersubjets.eval_auto_bounds( "central", BTagEntry::FLAV_UDSG, JETS[0].subjet1.Eta(), JETS[0].subjet1.Pt() );
			  subjet12SF = readersubjets.eval_auto_bounds( "up", BTagEntry::FLAV_UDSG, JETS[0].subjet1.Eta(), JETS[0].subjet1.Pt() );
			  subjet12SF = readersubjets.eval_auto_bounds( "down", BTagEntry::FLAV_UDSG, JETS[0].subjet1.Eta(), JETS[0].subjet1.Pt() );
			}


			if( JETS[1].subjet0PartonFlavor > 4 ) {
			  subjet21SF = readersubjets.eval_auto_bounds( "central", BTagEntry::FLAV_B, JETS[1].subjet0.Eta(), JETS[1].subjet0.Pt() );
			  subjet21SF = readersubjets.eval_auto_bounds( "up", BTagEntry::FLAV_B, JETS[1].subjet0.Eta(), JETS[1].subjet0.Pt() );
			  subjet21SF = readersubjets.eval_auto_bounds( "down", BTagEntry::FLAV_B, JETS[1].subjet0.Eta(), JETS[1].subjet0.Pt() );
			}
			else if( 5. > JETS[1].subjet0PartonFlavor && JETS[1].subjet0PartonFlavor > 0. ) {
			  subjet21SF = readersubjets.eval_auto_bounds( "central", BTagEntry::FLAV_C, JETS[1].subjet0.Eta(), JETS[1].subjet0.Pt() );
			  subjet21SF = readersubjets.eval_auto_bounds( "up", BTagEntry::FLAV_C, JETS[1].subjet0.Eta(), JETS[1].subjet0.Pt() );
			  subjet21SF = readersubjets.eval_auto_bounds( "down", BTagEntry::FLAV_C, JETS[1].subjet0.Eta(), JETS[1].subjet0.Pt() );
			}
			else {
			  subjet21SF = readersubjets.eval_auto_bounds( "central", BTagEntry::FLAV_UDSG, JETS[1].subjet0.Eta(), JETS[1].subjet0.Pt() );
			  subjet21SF = readersubjets.eval_auto_bounds( "up", BTagEntry::FLAV_UDSG, JETS[1].subjet0.Eta(), JETS[1].subjet0.Pt() );
			  subjet21SF = readersubjets.eval_auto_bounds( "down", BTagEntry::FLAV_UDSG, JETS[1].subjet0.Eta(), JETS[1].subjet0.Pt() );
			}


			if( JETS[1].subjet1PartonFlavor > 4 ) {
			  subjet22SF = readersubjets.eval_auto_bounds( "central", BTagEntry::FLAV_B, JETS[1].subjet1.Eta(), JETS[1].subjet1.Pt() );
			  subjet22SF = readersubjets.eval_auto_bounds( "up", BTagEntry::FLAV_B, JETS[1].subjet1.Eta(), JETS[1].subjet1.Pt() );
			  subjet22SF = readersubjets.eval_auto_bounds( "down", BTagEntry::FLAV_B, JETS[1].subjet1.Eta(), JETS[1].subjet1.Pt() );
			}
			else if( 5. > JETS[1].subjet1PartonFlavor && JETS[1].subjet1PartonFlavor > 0. ) {
			  subjet22SF = readersubjets.eval_auto_bounds( "central", BTagEntry::FLAV_C, JETS[1].subjet1.Eta(), JETS[1].subjet1.Pt() );
			  subjet22SF = readersubjets.eval_auto_bounds( "up", BTagEntry::FLAV_C, JETS[1].subjet1.Eta(), JETS[1].subjet1.Pt() );
			  subjet22SF = readersubjets.eval_auto_bounds( "down", BTagEntry::FLAV_C, JETS[1].subjet1.Eta(), JETS[1].subjet1.Pt() );
			}
			else {
			  subjet22SF = readersubjets.eval_auto_bounds( "central", BTagEntry::FLAV_UDSG, JETS[1].subjet1.Eta(), JETS[1].subjet1.Pt() );
			  subjet22SF = readersubjets.eval_auto_bounds( "up", BTagEntry::FLAV_UDSG, JETS[1].subjet1.Eta(), JETS[1].subjet1.Pt() );
			  subjet22SF = readersubjets.eval_auto_bounds( "down", BTagEntry::FLAV_UDSG, JETS[1].subjet1.Eta(), JETS[1].subjet1.Pt() );
			}
...
void RUNBoostedAnalysis::beginJob() {
	...
	if ( mkTree ) {
		...
		RUNAtree->Branch( "jet1CSVv2SF", &jet1CSVv2SF, "jet1CSVv2SF/F" );		
		RUNAtree->Branch( "jet1CSVv2SFUp", &jet1CSVv2SFUp, "jet1CSVv2SFUp/F" );
		RUNAtree->Branch( "jet1CSVv2SFDn", &jet1CSVv2SFDn, "jet1CSVv2SFDn/F" );
		RUNAtree->Branch( "jet1CMVAv2SF", &jet1CMVAv2SF, "jet1CMVAv2SF/F" );		
		RUNAtree->Branch( "jet1CMVAv2SFUp", &jet1CMVAv2SFUp, "jet1CMVAv2SFUp/F" );
		RUNAtree->Branch( "jet1CMVAv2SFDn", &jet1CMVAv2SFDn, "jet1CMVAv2SFDn/F" );
		RUNAtree->Branch( "jet1Flavor", &jet1Flavor, "jet1Flavor/F" );
		...
		RUNAtree->Branch( "jet2CSVv2SF", &jet2CSVv2SF, "jet2CSVv2SF/F" );		
		RUNAtree->Branch( "jet2CSVv2SFUp", &jet2CSVv2SFUp, "jet2CSVv2SFUp/F" );
		RUNAtree->Branch( "jet2CSVv2SFDn", &jet2CSVv2SFDn, "jet2CSVv2SFDn/F" );
		RUNAtree->Branch( "jet2CMVAv2SF", &jet2CMVAv2SF, "jet2CMVAv2SF/F" );		
		RUNAtree->Branch( "jet2CMVAv2SFUp", &jet2CMVAv2SFUp, "jet2CMVAv2SFUp/F" );
		RUNAtree->Branch( "jet2CMVAv2SFDn", &jet2CMVAv2SFDn, "jet2CMVAv2SFDn/F" );
		RUNAtree->Branch( "jet2Flavor", &jet2Flavor, "jet2Flavor/F" );
		...
		RUNAtree->Branch( "subjet11SFUp", &subjet11SFUp, "subjet11SFUp/F" );
		RUNAtree->Branch( "subjet11SFDn", &subjet11SFDn, "subjet11SFDn/F" );
		...
		RUNAtree->Branch( "subjet12SFUp", &subjet12SFUp, "subjet12SFUp/F" );
		RUNAtree->Branch( "subjet12SFDn", &subjet12SFDn, "subjet12SFDn/F" );
		...
		RUNAtree->Branch( "subjet21SFUp", &subjet21SFUp, "subjet21SFUp/F" );
		RUNAtree->Branch( "subjet21SFDn", &subjet21SFDn, "subjet21SFDn/F" );
		...
		RUNAtree->Branch( "subjet22SFUp", &subjet22SFUp, "subjet22SFUp/F" );
		RUNAtree->Branch( "subjet22SFDn", &subjet22SFDn, "subjet22SFDn/F" );
...
void RUNBoostedAnalysis::fillDescriptions(edm::ConfigurationDescriptions & descriptions) {
	...
	desc.add<InputTag>("subjetCMVAv2", 	InputTag("subjetsAK8CHS:subjetAK8CHSCMVAv2"));
        ...
        desc.add<InputTag>("jetFlavor",      InputTag("jetsAK8CHS:jetAK8CHSPartonFlavour"));
	...
	desc.add<InputTag>("genPart",         InputTag("filteredPrunedGenParticles"));
