...
private:
  ...
  TH1F* h_softDropMass = new TH1F("softDropMass",";softDrop mass",50,0,500);
...
jetAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for( edm::View<pat::Jet>::const_iterator jet_iter = jets->begin();
       jet_iter !=jets->end(); ++jet_iter)
    {
      ...
      float fullDiscriminant=0, qgLikelihood=0, tau1=0, tau2=0, tau3=0, tau4=0, tau5=0, QjetsVolatility=0, softDropMass=0, prunedMass=0, trimmedMass=0, filteredMass=0, massDropMass=0, topJetMass, hepTopJetMass;
	...
	h_softDropMass->Fill(softDropMass);
	...
	tau1 = jet_iter->userFloat("NjettinessAK8CHS:tau1");
	tau2 = jet_iter->userFloat("NjettinessAK8CHS:tau2");
	tau3 = jet_iter->userFloat("NjettinessAK8CHS:tau3");
	tau4 = jet_iter->userFloat("NjettinessAK8CHS:tau4");
	tau5 = jet_iter->userFloat("NjettinessAK8CHS:tau5");
	
	QjetsVolatility = jet_iter->userFloat("QJetsAdderAK8:QjetsVolatility");
	
	prunedMass = jet_iter->userFloat("ak8PFJetsCHSPrunedLinks");
	softDropMass = jet_iter->userFloat("ak8PFJetsCHSSoftDropLinks");
	trimmedMass = jet_iter->userFloat("ak8PFJetsCHSTrimmedLinks");
	filteredMass = jet_iter->userFloat("ak8PFJetsCHSFilteredLinks");
	
	massDropMass = jet_iter->userFloat("ak8PFJetsCHSMassDropFilteredLinks");
	topJetMass = jet_iter->userFloat("cmsTopTagPFJetsCHSLinksAK8");
	hepTopJetMass = jet_iter->userFloat("hepTopTagPFJetsCHSLinksAK8");
	...
	h_topJetMass->Fill(topJetMass);
      ...
      else if (jets_->label().compare("patJetsCA8withSubjets")==0 ){
      //else if (jets_->label().compare("patJetsAK8PFCHS")==0 ){
	std::cout << jet_iter->numberOfDaughters() << std::endl;
	      for( size_t q=0; q< jet_iter->numberOfDaughters(); q++){

		      std::cout<< "subjet" << q << " pt : "<< jet_iter->daughter(q)->pt() << std::endl;
	      }
	...
	h_topJetMass->Fill(topJetMass);
      ...
      else if (jets_->label().compare("patJetsCA15PFCHS")==0 ){
	
	tau1 = jet_iter->userFloat("NjettinessCA15:tau1");
	tau2 = jet_iter->userFloat("NjettinessCA15:tau2");
	tau3 = jet_iter->userFloat("NjettinessCA15:tau3");
	tau4 = jet_iter->userFloat("NjettinessCA15:tau4");
	tau5 = jet_iter->userFloat("NjettinessCA15:tau5");
	
	QjetsVolatility = jet_iter->userFloat("QJetsAdderCA15:QjetsVolatility");
	
	prunedMass = jet_iter->userFloat("ca15PFJetsCHSPrunedLinks");
	trimmedMass = jet_iter->userFloat("ca15PFJetsCHSTrimmedLinks");
	filteredMass = jet_iter->userFloat("ca15PFJetsCHSFilteredLinks");
	
	massDropMass = jet_iter->userFloat("ca15PFJetsCHSMassDropFilteredLinks");
	topJetMass = jet_iter->userFloat("cmsTopTagPFJetsCHSLinksCA15");
	hepTopJetMass = jet_iter->userFloat("hepTopTagPFJetsCHSLinksCA15");
	...
	h_topJetMass->Fill(topJetMass);
      ...
      else if (jets_->label().compare("ca8PFJetsPuppi")==0 ){
      }
      

      //if ( pt > 100 ) {
      std::cout<<"jet algo: "<< jets_->label() <<  std::endl;
      ...
      std::cout<<"massDropFilteredMass: "<< massDropMass<<std::endl;

      std::cout<<"topJetMass:"<<topJetMass<<std::endl;
      std::cout<<"hepTopJetMass:"<< hepTopJetMass<<std::endl;
