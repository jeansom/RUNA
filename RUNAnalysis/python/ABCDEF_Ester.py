...
# Calculates the transfer factors using New Estimate 2.0
### Dists: distributions to add
### DistsSub: distributions to subtract
### masses: mass bins for the TF
### detaBins: bins to use to fit the TF in each mass bin
def ABCDEF_Est(Dists, DistsSub, masses, detaBins):
    ABCDEF_EstFile = TFile( "TCutPlots/ABCDEF_EstFile.root", "RECREATE" )

    # 3d plots (mass ave vs mass asym vs delta eta)
    ABCDEF = TH3F("H3","",50, 0., 1., 50, 0., 5., 60, 50., 350.)
    ABCDEFSUB = TH3F("H3SUB","",50, 0., 1., 50, 0., 5., 60, 50., 350.)

    for i in Dists: quick3dplot(i.File, "BoostedAnalysisPlots/RUNATree", ABCDEF, "prunedMassAsym", "deltaEtaDijet", "prunedMassAve", "jet1Tau21<0.45&jet2Tau21<0.45", i.weight)
    for i in DistsSub: quick3dplot(i.File, "BoostedAnalysisPlots/RUNATree", ABCDEFSUB, "prunedMassAsym", "deltaEtaDijet", "prunedMassAve", "jet1Tau21<0.45&jet2Tau21<0.45", i.weight)

    ABCDEF.Add(ABCDEFSUB, -1)

    # Bunch of arrays, way too many
    AList, CList, VList, TList, TEHList, TELList, MList, EHList, ELList = ([] for i in xrange(9))
    ...
    for mass in masses:
        ...
        Bx1, Bx2, By1, By2 = ([] for i in xrange(4))
        Dx1, Dx2, Dy1, Dy2 = ([] for i in xrange(4))
        
        # Find B/D in each deta bin
        ...
        B, D, EYH, EYL, EXH, EXL = ([] for i in xrange(6))
        BSum, DSum = 0, 0
...
#        G.Write("BOverDDeta_"+str(mass[0]))
        ...
        # Save B/D fit for each mass bin
        G.SetTitle("")
    ...
    # Writes important arrays to a file for later use
    with open("ABCDEF_Est_output.py", "w") as f:
        ...
        del AList, CList, VList, TList, TEHList, TELList, EHList, ELList, MList, G
