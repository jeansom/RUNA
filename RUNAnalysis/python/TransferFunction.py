...
class TransferFunction:
	...
	def Converter( self,fitter ):
		...
		for i in [self.ErrUp, self.ErrDn]:
			i.SetLineStyle(2)
	def bkgEstFunction( self, rootFile, nameInRoot, rootFile2, nameInRoot2, xmin, xmax, rebinX, labX, labY, log, version, binning, grooming, plotFolder, data=False, subtract=False, Norm=False ):
		...
		if isinstance( rootFile, dict):
			...
			if isinstance( rootFile2, dict):
				...
				for bkgSam in rootFile2:
					bkgHistos[ bkgSam+'_A' ] = rootFile2[ bkgSam ][0].Get( nameInRoot+'__A' )
			else: 
				print "file2"
				bkgSam = 'bkg'
				bkgHistos[ bkgSam+'_A' ] = rootFile2.Get( nameInRoot+'__A' )
		else:
			print "file"
			bkgSam = 'bkg'
			bkgHistos[ bkgSam+'_A' ] = rootFile.Get( nameInRoot+'__A' )
			bkgHistos[ bkgSam+'_B' ] = rootFile.Get( nameInRoot+'__B' )
			bkgHistos[ bkgSam+'_C' ] = rootFile.Get( nameInRoot+'__C' )
			bkgHistos[ bkgSam+'_D' ] = rootFile.Get( nameInRoot+'__D' )
...
#		hBkgSamples_A[ "WZ" ] = bkgHistos[ "WZ_A" ].Clone()
		hBkgSamples_A[ "W + Jets" ] = bkgHistos[ "WJetsToQQ_D" ].Clone()
		hBkgSamples_A[ "tt + Jets" ] = bkgHistos[ "TTJets_D" ].Clone()
		hBkgSamples_A[ "QCD" ] = bkgHistos[ "QCDPtAll_D" ].Clone()
...
#		hBkgSamples_A[ "All MC Signal Region" ].Add(bkgHistos[ "WJetsToQQ_A" ])
		...
		if subtract and not data and isinstance( rootFile, dict):
			...
			hBkg_B.Add( rootFile['WJetsToQQ'][0].Get( nameInRoot+'__B' ), -1 )
			hBkg_C.Add( rootFile['WJetsToQQ'][0].Get( nameInRoot+'__C' ), -1 )
			hBkg_D.Add( rootFile['WJetsToQQ'][0].Get( nameInRoot+'__D' ), -1 )
...
#		self.MakeConvFactor()
		...
		for b in xrange(11,71):
			hBkg_Est.SetBinContent(b, hBkg_C.GetBinContent(b) * self.fit.Eval( float(b*4) ) )
			hBkg_Est_Up.SetBinContent(b, hBkg_C.GetBinContent(b) * self.ErrUp.Eval( float(b*4) ) )
			hBkg_Est_Dn.SetBinContent(b, hBkg_C.GetBinContent(b) * self.ErrDn.Eval( float(b*4) ) )
		...
		if subtract and not data and isinstance( rootFile, dict):
			...
			WJetsToQQ_A = rootFile['WJetsToQQ'][0].Get( nameInRoot+'__A' )
			hBkg_Est.Add( WJetsToQQ_A )
			hBkg_Est_Up.Add( WJetsToQQ_A )
			hBkg_Est_Dn.Add( WJetsToQQ_A )
		elif subtract and data and isinstance( rootFile2, dict):
			TTJets_A = rootFile2['TTJets'][0].Get( nameInRoot+'__A' )
			hBkg_Est.Add( TTJets_A )
			hBkg_Est_Up.Add( TTJets_A )
			hBkg_Est_Dn.Add( TTJets_A )
			WJetsToQQ_A = rootFile2['WJetsToQQ'][0].Get( nameInRoot+'__A' )
			hBkg_Est.Add( WJetsToQQ_A )
			hBkg_Est_Up.Add( WJetsToQQ_A )
			hBkg_Est_Dn.Add( WJetsToQQ_A )
		...
		Signal = TFile.Open("/cms/data28/jeans/CMSSW_7_6_3_patch2/src/RUNA/RUNAnalysis/test/Rootfiles/102216Tau32Inverse/RUNBkgEstimationUDD323_pruned_RPVStopStopToJets_UDD323_M-100_low_v05p6.root")
		hSignalA = Signal.Get( nameInRoot+'__A' )

		hRatiohBkg = ratioPlots( hBkg_A, hBkg_Est )
		if subtract and data:
			makePlots( nameInRoot, hBkg_A, 'All MCs SR', hBkg_Est, 'DATA ABCD Pred', 5, xmin, xmax, hRatiohBkg, "MC SR/ABCD Pred", '', 'MCBkgTransfer_Log_Subtract', binning, 'v05p5', grooming, plotFolder, True )
			StackHistos( nameInRoot, hBkgSamples_A, 'All MCs SR', hBkg_Est, 'DATA ABCD Pred', ErrHistos, 5, xmin, xmax, hRatiohBkg, "MC SR/ABCD Pred", '', 'MCBkgTransfer_Log_Subtract', binning, 'v05p5', grooming, plotFolder, True )
		elif subtract and not data:
			makePlots( nameInRoot, hBkg_A, 'All MCs SR', hBkg_Est, 'ABCD Pred', 5, xmin, xmax, hRatiohBkg, "MC SR/ABCD Pred", '', 'MCBkgTransfer_Log_Subtract', binning, 'v05p5', grooming, plotFolder, True )
			StackHistos( nameInRoot, hBkgSamples_A, 'All MCs SR', hBkg_Est, 'ABCD Pred', ErrHistos, 5, xmin, xmax, hRatiohBkg, "MC SR/ABCD Pred", '', 'MCBkgTransfer_Log_Subtract', binning, 'v05p5', grooming, plotFolder, True )
		elif not subtract and data:
			makePlots( nameInRoot, hBkg_A, 'All MCs SR', hBkg_Est, 'DATA ABCD Pred', 5, xmin, xmax, hRatiohBkg, "MC SR/ABCD Pred", '', 'MCBkgTransfer_Log', binning, 'v05p5', grooming, plotFolder, True )
			StackHistos( nameInRoot, hBkgSamples_A, 'All MCs SR', hBkg_Est, 'DATA ABCD Pred', ErrHistos, 5, xmin, xmax, hRatiohBkg, "MC SR/ABCD Pred", '', 'MCBkgTransfer_Log', binning, 'v05p5', grooming, plotFolder, True )

		else:
			makePlots( nameInRoot, hBkg_A, 'All MCs SR', hBkg_Est, 'ABCD Pred', 5, xmin, xmax, hRatiohBkg, "MC SR/ABCD Pred", '', 'MCBkgTransfer_Log', binning, 'v05p5', grooming, plotFolder, True )
#			StackHistos( nameInRoot, hBkgSamples_A, 'All MCs SR', hBkg_Est, 'ABCD Pred', ErrHistos, 5, xmin, xmax, hRatiohBkg, "MC SR/ABCD Pred", '', 'MCBkgTransfer_Log', binning, 'v05p5', grooming, plotFolder, True )
			StackHistos( nameInRoot, hBkgSamples_A, 'All MCs', hSignalA, 'Signal', ErrHistos, 5, xmin, xmax, hRatiohBkg, '', '', 'MCBkgTransfer_Log', binning, 'v05p5', grooming, plotFolder, True )
...
#		print self.fit.GetParameter(2)
		...
		self.G.Draw("A*")
		self.G.GetXaxis().SetTitle( "Average pruned mass [GeV]" )
		self.G.GetYaxis().SetTitle( "Ratio B/D" )
#		self.G.GetYaxis().SetRangeUser( 0, 3 )
		gStyle.SetOptFit(1111)
		
		self.G.Draw("A*")
		self.ErrUp.SetRange(0,350)
		self.ErrDn.SetRange(0,350)
		self.ErrUp.Draw("same")
		self.ErrDn.Draw("same")
		if subtract:
			canBD.SaveAs( plotFolder+"BkgEstWithTransferFunctionPlotSubtract.png" )
		else:
			canBD.SaveAs( plotFolder+"BkgEstWithTransferFunctionPlot.png" )
