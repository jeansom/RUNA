...
# Returns the bin numbers for the x and y axis of a 2d plot
def GetBins( h, x1, x2, y1, y2 ):
    return h.GetXaxis().FindBin(x1), h.GetXaxis().FindBin(x2), h.GetYaxis().FindBin(y1), h.GetYaxis().FindBin(y2)

# I don't use, used if you want to use data errors (ignore weights)
def FixWeights( h ):
    for bin in xrange(1, h.GetNbinsX()+1 ):
        h.SetBinContent(bin, h.GetBinContent(bin))

# Returns errors on fit
### bins: bins to find errors for
### fit: fit
### fitResult: TFitResultPtr for Fit
...
# Returns errors on est, pull plot
### Est: bkg estimate
### Pull2: pull plot
### DATA: Signal Region
### ESTUP, ESTDN: Upper error on est, lower error on est
### A: Antitag region
def PullErrors(EST, Pull2, DATA, ESTUP, ESTDN, A):
    ...
    for i in xrange(1, EST.GetNbinsX()+1):
        ...
        if A.GetBinContent(i)==0: 
            ...
            a = A.GetBinError(i)*EST.GetBinContent(i)/A.GetBinContent(i)
