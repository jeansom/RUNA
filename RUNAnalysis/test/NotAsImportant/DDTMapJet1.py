...
def DisplayDDT(DDT):
	...
	DDT.Draw("COLZ")
	DDT.Draw("CONT3 same")
	C.Print("DDT_Map_jet1Tau21DDT_"+CUT+".png")
...
def DrawRho(DDT):
	...
	for i in xrange(len(GA)):
		...
		GA[i].Draw("C")
	C.SaveAs("DDT_Map_jet1Tau21DDT_"+CUT+".png")
def FriendTree(DDT, Bkgs, CUT):
	for B in Bkgs:
		...
		FF = TFile(NAMEF+"_friendDDT_jet1Tau21DDT_"+CUT+".root", "RECREATE")
		TF = TTree("RUNATreeFriend", "")
		TF.SetName("TF")
		DDTVal = array( 'd', [0] )
		TF.Branch("DDT_jet1Tau21DDT_"+CUT, DDTVal, "DDT/D")
		for j in range(0,n):
			T1.GetEntry(j)
			fraction = 10.*j/(1.*n)
...
Fout = TFile("PhotonDDTs_jet1Tau21DDT_"+CUT+".root", "recreate")
Fout.cd()
DDT.Write()
Fout.Close()
