...
#    NBins = int((())/binWidth)
    ...
    var_arrayMass = [ rho, Xcut[0], NBins, -40., 0., Xcut[2], Xcut[3], Xcut[4] ] # For making B,D plot
#    for i in xrange( 0, NBins ):
#        binsMass.append( [ var_arrayMass[3]+binWidth*i, var_arrayMass[3]+binWidth*(i+1) ] )
    binsMass.append( [-40, -10] )
    i = -10
    while i < 1.1:
        binsMass.append( [i, i+.1] )
        i = i+.1

    #### Form of fit
    FMass = SigmoidFit(prelimFit,-40, 0,"Mass","SEMR") # Nominal fit
    FMass1 = SigmoidFit(prelimFit, -40, 0,"Mass1","SEMR") # Nominal fit again, used if staying blinded
    ...
    # Nominal
    MakeFitPlots( EstMass, FMass, binsMass, "#rho", Xcut[0], var_arrayMass, chanCutsTemp, Ycut[0]+">"+Ycut[1], cut, 0, "", "", "outputs/"+directory+method+"/Mass"+chan+"_"+scaleName, binBoundaries, False, False ) # Creates the 2D plot, ratio, and ratio fit
