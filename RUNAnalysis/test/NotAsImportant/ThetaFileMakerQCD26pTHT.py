...
def ThetaFileMaker( chan, scaleName, chanCuts, bins, minBin, maxBin, Xcut, Ycut, tau21, prelimFit, fitBins, HT, Dists, DistsSub, DistSig, isMC=True, log=True ):
    ...
    binsMass = []
    binWidth = 180
    NBins = int(((3000-300))/binWidth)
    var_arrayMass = [ "jet1Pt+jet2Pt", Xcut[0], NBins, 300., 3000., Xcut[2], Xcut[3], Xcut[4] ] # For making B,D plot
    for i in xrange( 0, NBins ):
        binsMass.append( [ var_arrayMass[3]+binWidth*i, var_arrayMass[3]+binWidth*(i+1) ] )

    #### Form of fit
    FMass = SigmoidFit(prelimFit,300, 3000,"Mass","SEMR") # Nominal fit
    FMass1 = SigmoidFit(prelimFit, 300, 3000,"Mass1","SEMR") # Nominal fit again, used if staying blinded
    ...
    # Nominal
    MakeFitPlots( EstMass, FMass, binsMass, "jet1Pt+jet2Pt", Xcut[0], var_arrayMass, chanCutsTemp, Ycut[0]+">"+Ycut[1], cut, 0, "", "", "outputs/"+directory+method+"/Mass"+chan+"_"+scaleName, binBoundaries, False, False ) # Creates the 2D plot, ratio, and ratio fit
    MakeEstPlots( EstMass, "prunedMassAve", "Average Mass", binBoundaries, presel+"&"+chanCuts+"&"+anticuts, presel+"&"+chanCuts+"&"+cuts, "prunedMassAve", "", "outputs/"+directory+method+"/Mass"+chan+"_"+scaleName, rhoBinBoundaries, False, isMC, False, log ) # Makes the actual estimation
