...
def ThetaFileMaker( chan, scaleName, chanCuts, bins, minBin, maxBin, Xcut, Ycut, tau21, prelimFit, fitBins, HT, Dists, DistsSub, DistSig, isMC=True, log=True ):
    ...
    if fit:
        ...
        for i in xrange( int(var_arrayMass[3]), 200 ):
            if i%5 == 0: binsMass.append( [ i, i+5] )
        for i in xrange( 200, 350 ):
            if i%25 == 0: binsMass.append( [i, i+25] )

    #### Form of fit
    FMass = LinearFit(prelimFit,60, 350,"Mass","SEMR") # Nominal fit
    FMass1 = LinearFit(prelimFit,60, 350,"Mass1","SEMR") # Nominal fit again, used if staying blinded
...
#        i=i+dBin
    ...
    # Nominal
    MakeFitPlots( EstMass, FMass, binsMass, "prunedMassAve", Xcut[0], var_arrayMass, chanCutsTemp, Ycut[0]+">"+Ycut[1], cut, 0, "", "", "outputs/"+directory+method+"/Mass"+chan+"_"+scaleName, rhoBinBoundaries, fit, False ) # Creates the 2D plot, ratio, and ratio fit
    MakeEstPlots( EstMass, "jet1Eta", "jet1Eta", binBoundaries, presel+"&"+chanCuts+"&"+anticuts, presel+"&"+chanCuts+"&"+cuts, "prunedMassAve", "", "outputs/"+directory+method+"/Mass"+chan+"_"+scaleName, rhoBinBoundaries, fit, isMC, False, log ) # Makes the actual estimation
...
prelimFit = [0,0,0,0,0]
#tau21 = "(jet1Tau21-DDT_20_tau21)<0&(jet2Tau21-DDT_20_tau21)<0"
...
ThetaFileMaker( name, scaleName, chanCuts, bins, 60, 350, Xcut, Ycut, tau21, prelimFit, 5, HT, Dists, DistsSub, [], isMC, log )
