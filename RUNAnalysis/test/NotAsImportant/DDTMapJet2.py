...
def ComputeDDT(name, point, nPtBins, nRhoBins, H, CUT):
...
def DisplayDDT(DDT):
	...
	DDT.Draw("COLZ")
	DDT.Draw("CONT3 same")
	C.Print("DDT_Map_jet2Tau21DDT_"+CUT+".png")
...
def DrawRho(DDT):
	...
	for i in xrange(len(GA)):
		...
		GA[i].Draw("C")
	C.SaveAs("DDT_Map_jet2Tau21DDT_"+CUT+".png")
def FriendTree(DDT, Bkgs, CUT):
	for B in Bkgs:
		F = TFile(B[0])
		...
		FF = TFile(NAMEF+"_friendDDT_jet2Tau21DDT_"+CUT+".root", "RECREATE")
		TF = TTree("RUNATreeFriend", "")
		TF.SetName("TF")
		DDTVal = array( 'd', [0] )
		TF.Branch("DDT_jet2Tau21DDT_"+CUT, DDTVal, "DDT/D")
		for j in range(0,n):
			T1.GetEntry(j)
			fraction = 10.*j/(1.*n)
			...
			PT = (T1.jet2Pt)
			PRERHO = (T1.jet2PrunedMass*T1.jet2PrunedMass/(T1.jet2Pt*T1.jet2Pt))
			if PRERHO <= 0: RHO = 0
			else: RHO = log(PRERHO)
...
EFF = 0.182784721132
PT = "(jet2Pt)"
RHO = "log(jet2PrunedMass*jet2PrunedMass/(jet2Pt*jet2Pt))"
TAU21 = "jet2Tau21"
for B in Bkgs:
	weight = "puWeight*"+str(scaleFactor(B[1]))+"*36555.21/15"
	quick3dplot(B[0], "BoostedAnalysisPlotsPuppi/RUNATree", H3, RHO, PT, TAU21, "jet2Tau21>0.", weight)
print '*'*20+"COMPUTING DDT"+'*'*20
DDT = ComputeDDT("DDT", EFF, PB, RB, H3, CUT)
DisplayDDT(DDT)
...
FriendTree(DDT, Bkgs, CUT)

Fout = TFile("PhotonDDTs_jet2Tau21DDT_"+CUT+".root", "recreate")
Fout.cd()
DDT.Write()
Fout.Close()
