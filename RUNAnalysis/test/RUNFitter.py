...
def createPseudoExperiment():
	...
	hMainPSE.Sumw2()
	hMainPSE.Draw()
	c1.SaveAs(outputDir+hist+"_PseudoExperiment_Fit_ResolvedAnalysis."+args.extension)
	del c1
...
def rootFitter( inFileBkg, hist, scale, fitFunction, fitParam, minX, maxX, rebinX, plot, log=True ):
	...
	if plot:
		...
		hBkg.SetTitle("")
		hBkg.Draw()
		c1.SaveAs(outputDir+hist.replace('ResolvedAnalysisPlots/','')+"_"+args.process+"_"+fitFunction.GetName()+"Fit_ResolvedAnalysis."+args.extension)
		del c1
	
	return [ fitParameters, numEvents, binContents, binError ]

def FitterCombination( inFileData, inFileBkg, inFileSignal, hist, scale, bkgFunction, fitParam, minX, maxX, rebinX ):
	"""docstring for FitterCombination"""

	### Fit QCD MC
	print "|----> Fitting MC QCD"
	BkgParameters = rootFitter( inFileBkg, hist, scale, P4, fitParam, minX, maxX, rebinX, True )
	...
	if not 'MC' in args.process:
		print "|----> Fitting Data"
		DataParameters = rootFitter( inFileData, hist, scale, P4, bkgParameters, minX, maxX, rebinX, False )
	...
	hist = hist.replace('ResolvedAnalysisPlots/','')
	
	mainP4 = TF1("mainP4", "[0]*pow(1-(x/13000.0),[1])/pow(x/13000.0,[2]+([3]*log(x/13000.)))", minX, maxX);
	mainP4.SetParameter(0, dataParameters[0] if not 'MC' in args.process else bkgParameters[0] )				
	mainP4.SetParameter(1, dataParameters[1] if not 'MC' in args.process else bkgParameters[1] )				
	mainP4.SetParameter(2, dataParameters[2] if not 'MC' in args.process else bkgParameters[2] )				
	mainP4.SetParameter(3, dataParameters[3] if not 'MC' in args.process else bkgParameters[3] )				
	...
	hResidual.Draw("e")
	line.Draw("same")
	c3.SaveAs("Plots/"+hist+"_"+args.process+"_"+args.version+"FitP4_ResolvedAnalysis."+args.extension)
	del c3

	#return P4GausParameters

def rooFitter( inFileBkg, inFileSignal, hist, outputRootFile, minX, maxX ):
	...
	P4GausParameters = FitterCombination( inFileBkg, inFileSignal, hist, P4, minX, maxX )
	bkgAcc = P4GausParameters[7]
	#sigAcc = P4GausParameters[8]
	
	hSignal = inFileSignal.Get(hist)
 ...
 # -----------------------------------------
        ...
        datacard = open('/afs/cern.ch/work/a/algomez/Substructure/CMSSW_7_1_5/src/MyLimits/datacard_RPVStop'+str(args.mass)+'.txt','w')
...
#	c1.SaveAs('Plots/'+hist+"_QCD_FitP4Gaus_rooFit."+args.extension)
#	del c1
#
#	'''
...
#	x2frame.Draw()
#	c2.SaveAs('Plots/'+hist+"_RPVSt100tojj_FitP4Gaus_rooFit."+args.extension)
#	del c2
#	'''
#
#	c1 = TCanvas('c1', 'c1',  10, 10, 750, 500 )
...
#	xframe.Draw()
#	c1.SaveAs('Plots/'+hist+"_PseudoExp_FitP4Gaus_rooFit."+args.extension)
#	del c1
#	'''
#
#	####### QCD AS DATA
...
def rooFitterTree( inFileBkg, inFileSignal, inFileData, hist):
	...
	pdf.paramOn( xframe, RooFit.Layout(0.6,0.9,0.94))
	xframe.Draw()
	c1.SaveAs('Plots/'+hist+"_QCD_RPVSt100tojj_FitP4Gaus_rooFitTree."+args.extension)
	del c1
...
if __name__ == '__main__':
	...
	if args.miniTree: 
		filePrefix = 'Rootfiles/RUNMiniResolvedAnalysis_'
	...
	outputRootFile = '' #'/afs/cern.ch/work/a/algomez/Substructure/CMSSW_7_4_5_patch1/src/RUNA/RUNAnalysis/test/Rootfiles/workspace_QCD_RPVSt'+str(args.mass)+'tojj_FitP4Gaus_rooFit_'+lumi+'fb.root'
	...
	if 'full' in args.process:
		...
		p = Process( target=FitterCombination, args=( dataFile, bkgFile, signalFile, hist, scale, P4, [ 0.1, 100, 2, 0.1 ], minFit, maxFit, rebinX ))
	elif 'Data' in args.process:
		CMS_lumi.extraText = "Preliminary"
		p = Process( target=FitterCombination, args=( dataFile, bkgFile, '', hist, scale, P4, [ 0.1, 100, 2, 0.1 ], minFit, maxFit, rebinX ))
	elif 'MC' in args.process:
		p = Process( target=FitterCombination, args=( '', bkgFile, signalFile, hist, scale, P4, [ 1, 100, 1, 1 ], minFit, maxFit, rebinX ))
	...
	elif 'rooFit' in args.process:
		rooFitter( dataFile, signalFile, hist, outputRootFile, 200.0, 1000.0  )
		#rooFitter( bkgFile, signalFile, hist, outputRootFile, 250.0, 450.0  )
	else:
