...
def main():
  # usage description
  usage = "Usage: createCrabJobs.py [options] \nExample: ./createCrabJobs.py -w CRAB_Jobs -d datasetList.txt -c bTaggingSFConverter_cfg.py -t crab_template.py"
  ...
  for line in dataset_list_lines:
    ...
    if(line_elements[8] != '-'):
      ...
      crab_cfg_content = re.sub('config.Data.publishDBS ','#config.Data.publishDBS ',crab_cfg_content)
    ...
    if(line_elements[4] != '-'):
      ...
      crab_cfg_content = re.sub('config.Data.runRange','#config.Data.runRange',crab_cfg_content)
    if(line_elements[5] != '-'):
      ...
      crab_cfg_content = re.sub('config.Data.lumiMask','#config.Data.lumiMask',crab_cfg_content)
    ...
    crab_cfg_content = re.sub('WORKING_DIR',dataset[:100],crab_cfg_content)
    ...
    crab_cfg_name = os.path.join(cfg_files_dir,dataset + '_crab.py')
